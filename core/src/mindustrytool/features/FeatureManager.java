...
public class FeatureManager {
    ...
    public void setEnabled(Feature feature, boolean enabled) {
        ...
        applyEnabled(feature, enabled);
    }

    /**
     * Порт: вынесено из setEnabled для интеграции с категорией в настройках клиента —
     * там checkPref сам записывает значение в settings ДО коллбэка, из-за чего guard
     * current == enabled в setEnabled молча съедал бы onEnable/onDisable.
     */
    public void applyEnabled(Feature feature, boolean enabled) {
        Core.app.post(() -> {
            if (enabled) {
