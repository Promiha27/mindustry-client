...
/** Выбор тайла для мировой заливки/кисти: пол + стат.блок + оверлей + постройка. */
public class TileSelectDialog extends BaseDialog{
    ...
    public TileSelectDialog(){
        super("@scheme.select.tile");
        addCloseButton();

        Seq<Folder> folders = Seq.with(
            new Folder("scheme.select.floor", () -> floor, b -> b instanceof Floor && !(b instanceof OverlayFloor), b -> floor = b),
            new Folder("scheme.select.block", () -> block, b -> b instanceof Prop, b -> block = b),
            new Folder("scheme.select.overlay", () -> overlay, b -> b instanceof Floor, b -> overlay = b),
            new Folder("scheme.select.building", () -> building, b -> b.uiIcon != atlas.getRegionMap().get("error") || !(b instanceof Prop || b instanceof Floor || b.description == null), b -> building = b));
