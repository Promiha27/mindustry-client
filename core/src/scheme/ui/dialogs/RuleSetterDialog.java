...
/**
 * Изменение ЛЮБЫХ правил карты на лету (через рефлексию по Rules/TeamRule), с поиском,
 * пер-командными правилами и редакторами banned-списков. Работает у хоста напрямую,
 * на серверах с JSEval - через /js. У клиента нативно есть только просмотр правил
 * (mi2u MapInfo) и вшитый CustomRulesDialog в паузе, доступный лишь при allowEditRules.
 */
public class RuleSetterDialog extends BaseDialog{
    ...
    private void buildTeamSection(Table t, Team team, Rules.TeamRule tr){
        int tid = team.id;
    ...
    private void check(Table t, String key, boolean value, arc.func.Boolc onChange){
        if(!matchesSearch(key)) return;

        String label = bundle.has(key) ? bundle.get(key) : key;
        t.check(label, value, val -> {
            onChange.get(val);
            rebuild();
        }).pad(6).left();
        t.row();
    ...
    private void number(Table t, String key, float value, arc.func.Floatc onChange){
        if(!matchesSearch(key)) return;
    ...
    private void integer(Table t, String key, int value, arc.func.Intc onChange){
        if(!matchesSearch(key)) return;
