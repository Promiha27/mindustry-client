...
/**
 * Строительные инструменты Scheme Size: сброс предметов из здания, замена/снос цепочки
 * одинаковых блоков, подключение к энергосети, заливка области, квадрат, круг, пипетка тайла,
 * мировая заливка/кисть (через админ-инструменты) и возврат снесённого. У клиента аналогов
 * нет (инвентаризация: у qol только реконнект энергосетей и апгрейд конвейеров, у ванили -
 * прямоугольная заливка только для env-блоков).
 */
public class BuildingTools{
    ...
    private void drop(Tile tile, Item item){
        ...
        int maxAccepted = player.unit().type.itemCapacity;
        Call.requestItem(player, tile.build, item, maxAccepted);
        if(player.unit().stack.amount > 0) Call.dropItem(maxAccepted);
    }
    ...
    public void replace(int x, int y){
        ...
        }catch(Throwable ignored){} // слишком глубокая рекурсия на огромных полях - просто обрываем
    }
    ...
    public void remove(int x, int y){
        ...
        try{ iterate(tile); }catch(Throwable ignored){}
    }
    ...
    private void iterateTileSafe(Tile tile){
        try{ iterateTile(tile); }catch(Throwable ignored){}
    }
    ...
    public void connect(int x, int y, Cons2<Integer, Integer> callback){
        ...
        Building power = Units.closestBuilding(player.team(), x * tilesize, y * tilesize, 999999f, build -> build.power != null && ((build.tileX() < x - size || build.tileX() > x + size) || (build.tileY() < y - size || build.tileY() > y + size)));
    ...
    public void resized(){
        ...
        if(hudfrag != null && hudfrag.size != null) hudfrag.size.setText(String.valueOf(size));
    }
