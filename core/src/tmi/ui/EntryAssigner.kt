...
object EntryAssigner {
  ...
  fun assign() {
    ...
    run { //HUD entry
      Core.scene.root.addChild(object : ImageButton(Consts.tmi, Styles.cleari) {
        init {
          ...
          //дефолт поднят над BLUI-панелью Testing Utilities (та тоже в левом нижнем углу, ~2 ряда
          //по 40px); позиция всё равно перетаскивается и запоминается
          setPosition(
            Core.settings.getFloat("tmi_button_x", 0f),
            Core.settings.getFloat("tmi_button_y", Scl.scl(140f)),
            Align.bottomLeft
          )
    ...
    //preview switch (tmi_enable_preview) - в секции настроек TooManyItems.registerSettings
  }
