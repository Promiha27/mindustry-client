...
import helium.util.invoke
import mindustry.gen.Icon
import mindustry.graphics.Pal
import mindustry.ui.Styles

/**
 * Порт helium.ui.UIUtils (Helium, EB-wilson): единый стиль всплывающих панелей мод-менеджера -
 * showPane/showInput/showTip/showConfirm/showError/showException + разделительная линия line().
 */
object UIUtils{
    val cancelBut get() = ButtonEntry(Core.bundle["cancel"], Icon.cancel, clicked = { it.hide() })
    val closeBut get() = ButtonEntry(Core.bundle["misc.close"], Icon.cancel, clicked = { it.hide() })
            ...
            info.table{ but ->
                ...
                buttons.forEach{ entry ->
                    but.button("", Styles.none, Styles.flatt, 32f){
                        ...
                        entry.icon?.also{ i ->
                            val img = b.find<Image>{ e -> e is Image }
    ...
    fun Table.line(color: Color, horizon: Boolean, stroke: Float): Cell<Image> = image().color(color).also{
...
class ButtonEntry(
    val title: arc.func.Prov<String>,
    val icon: arc.func.Prov<Drawable>? = null,
