...
/**
 * Порт мода "Extra Editor" (KlasterX, v1.0, один файл) как нативный пакет клиента: утилиты
 * РЕДАКТОРА КАРТ - copy/cut/paste буфера тайлов (с поворотом/отражением и ghost-превью),
 * своя кисть с формами (круг/квадрат/ромб/крест) и рисованием линий, свой ластик, режим
 * замены блока на блок, продвинутая сетка с детализацией и список undo-операций с
 * превью и точечным откатом. Всё живёт ТОЛЬКО внутри MapEditorDialog - пересечений со
 * scheme building tools (те работают в игре) и остальными вшитыми пакетами нет.
 *
 * Адаптация по образцу HeliumMod/MI2UMod: без extends Mod, конструктор зовётся из
 * mindustry.client.Main.init() и только вешает слушатели; self-disable guard - при
 * установленном внешнем моде "extra-editor" вшитая копия молчит. Спрайты лежат в
 * core/assets/extraeditor/ и регистрируются в атлас вручную (паттерн MI2UMod.loadSprites);
 * iconByName и так падает обратно на ванильные иконки, если региона нет. Настройки -
 * секция «Extra Editor» общей вкладки «Моды» (ModsSettings) с оригинальными ключами
 * ee-ghost-opacity/ee-ghost-outline; поля инициализируются из Core.settings при загрузке
 * (в оригинале значения применялись только при движении слайдера - после рестарта висел
 * дефолт). Хардкод-строки UI переведены на бандл client.extraeditor.* (en+ru).
 * Reflection-доступ к MapEditor.stack/DrawOperation.array/MapView.zoom оставлен как в
 * оригинале: поля приватные, а расширять API ванильных файлов ради этого - лишний
 * merge-конфликт с upstream.
 */
public class ExtraEditorMod{
    ...
    public ExtraEditorMod(){
        //self-disable: рядом установлен настоящий Extra Editor - две копии дрались бы за
        //MapView-листенер и тулбар, вшитая уступает (общий паттерн всех вшитых пакетов)
        if(mods.locateMod("extra-editor") != null){
            Log.info("[extraeditor] External Extra Editor mod is also loaded - baked-in copy is standing down.");
            return;
        }
    ...
    /**
     * Регистрирует спрайты мода в атласе под их оригинальными именами (extraeditor-*).
     * У обычного мода это делает спрайт-пакер Mods; вшитая копия несёт png в
     * core/assets/extraeditor/ (иконки мелкие, оверхед несущественный).
     */
    private static void loadSprites(){
        for(String name : new String[]{"square", "circle", "diamond", "cross", "hide", "show", "grid-area"}){
            String region = "extraeditor-" + name;
            if(Core.atlas.has(region)) continue;
            arc.graphics.Texture tex = new arc.graphics.Texture(Core.files.internal("extraeditor/" + name + ".png"));
    ...
    private void registerSettings(){
        //подтяжка сохранённых значений: колбэки префов зовутся только при ИЗМЕНЕНИИ
        ghostOpacity = Core.settings.getInt("ee-ghost-opacity", 12) / 100f;
        showGhostOutline = Core.settings.getBool("ee-ghost-outline", true);
        mindustry.client.ui.ModsSettings.section("modsec-extraeditor", table -> {
            table.sliderPref("ee-ghost-opacity", 12, 0, 100, 1, v -> v + "%", v -> ghostOpacity = v / 100f);
            table.checkPref("ee-ghost-outline", true, v -> showGhostOutline = v);
        });
    }

    /** Строка UI из бандла client.extraeditor.* (оригинал держал английский хардкодом). */
    private static String bnd(String key){
        return Core.bundle.get("client.extraeditor." + key);
    }
    ...
    private void buildToolbar(){
        ...
        }).size(32f).tooltip(bnd("hideshow")).get();
        ...
        ImageButton select = toolButton(Icon.copy, bnd("select"), Mode.select);
        bindOptions(select, this::showCopyOptions);
        ImageButton cut = toolButton(Icon.trash, bnd("cut"), Mode.cut);
        bindOptions(cut, this::showCutOptions);
        menuBody.row();

        ImageButton paste = toolButton(Icon.paste, bnd("paste"), Mode.paste);
        ...
        }).tooltip(bnd("undolist")).size(44f).get();
        undoToggleBtn.update(() -> undoToggleBtn.setChecked(undoListOpen));
        menuBody.button(Icon.rotate, Styles.flati, this::rotateClipboard).tooltip(bnd("rotate")).disabled(b -> clipboard == null);
        menuBody.row();

        menuBody.button(Icon.flipX, Styles.flati, this::flipClipboardX).tooltip(bnd("flipx")).disabled(b -> clipboard == null);
        menuBody.button(Icon.flipY, Styles.flati, this::flipClipboardY).tooltip(bnd("flipy")).disabled(b -> clipboard == null);
        menuBody.row();

        ImageButton drawBtn = toolButton(Icon.pencil, bnd("draw"), Mode.draw);
        bindOptions(drawBtn, this::showDrawOptions);
        ImageButton drawLineBtn = toolButton(Icon.line, bnd("drawline"), Mode.drawLine);
        bindOptions(drawLineBtn, this::showDrawOptions);
        menuBody.row();

        ImageButton eraser = toolButton(Icon.eraser, bnd("eraser"), Mode.eraseOre);
        ...
        menuBody.stack(
            ...
            new Table(t -> {
                ...
                t.button(iconByName("extraeditor-grid-area", Icon.map), Styles.flati, () -> {
                    selectingGridArea = true;
                    setMode(Mode.none);
                }).size(14f).pad(1f).tooltip(bnd("gridarea"));
            })
        ).size(44f);
        menuBody.row();
        ...
        }).tooltip(bnd("replacemode")).size(44f).get();
        ...
        fromPickButton.addListener(new Tooltip(t -> t.label(() -> replaceFrom == null ? bnd("replfrom") : replaceFrom.localizedName)));
        ...
        toPickButton.addListener(new Tooltip(t -> t.label(() -> replaceTo == null ? bnd("replto") : replaceTo.localizedName)));
        toPickButton.update(() -> {
        ...
        menuBody.button(Icon.cancel, Styles.flati, () -> {
            ...
            replaceTo = null;
            replaceMode = false;
        }).tooltip(bnd("replreset")).size(44f);
        menuBody.row();

        menuBody.add(bnd("brushshape")).colspan(2).center().growX().row();
        menuBody.table(t -> {
            t.defaults().size(34f).pad(3f);
            t.center();
            ImageButton b1 = t.button(iconByName("extraeditor-circle", Icon.add), Styles.squareTogglei, () -> brushShape = Shape.circle).tooltip(bnd("circle")).get();
            b1.update(() -> b1.setChecked(brushShape == Shape.circle));
            ImageButton b2 = t.button(iconByName("extraeditor-square", Icon.add), Styles.squareTogglei, () -> brushShape = Shape.square).tooltip(bnd("square")).get();
            b2.update(() -> b2.setChecked(brushShape == Shape.square));
            t.row();
            ImageButton b3 = t.button(iconByName("extraeditor-diamond", Icon.add), Styles.squareTogglei, () -> brushShape = Shape.diamond).tooltip(bnd("diamond")).get();
            b3.update(() -> b3.setChecked(brushShape == Shape.diamond));
            ImageButton b4 = t.button(iconByName("extraeditor-cross", Icon.add), Styles.squareTogglei, () -> brushShape = Shape.cross).tooltip(bnd("cross")).get();
            b4.update(() -> b4.setChecked(brushShape == Shape.cross));
        }).colspan(2).center().growX().padLeft(24f);
        menuBody.row();

        menuBody.add(bnd("brushsize")).colspan(2).center().row();
        menuBody.table(t -> {
            t.table(field -> {
        ...
        menuBody.add(bnd("griddetail")).colspan(2).center().row();
        menuBody.table(t -> {
            t.table(field -> {
    ...
    private Table buildCopyOptionsTable(){
        ...
        pop.check(bnd("copyfloor"), copyFloor, v -> copyFloor = v).row();
        pop.check(bnd("copyore"), copyOre, v -> copyOre = v).row();
        pop.check(bnd("copyblocks"), copyBlock, v -> copyBlock = v).row();
    ...
    private Table buildPasteOptionsTable(){
        Table pop = new Table(Tex.button);
        pop.defaults().left().pad(3f);
        pop.check(bnd("overrideblocks"), overrideBlock, v -> overrideBlock = v).row();
        pop.check(bnd("overrideores"), overrideOre, v -> overrideOre = v).row();
        pop.check(bnd("smartpaste"), smartPasting, v -> smartPasting = v).row();
        pop.check(bnd("editorteam"), pasteUseEditorTeam, v -> pasteUseEditorTeam = v).row();
        return showOptionsPopup(pop);
    }
    ...
    private void showUndoList(){
        ...
        header.add(bnd("actions")).left();
        header.add().growX();
        header.button(Icon.refresh, Styles.clearNonei, () -> {
            if(undoListOpen) showUndoList();
        }).size(20f).tooltip(bnd("update"));
        pop.add(header).growX().row();

        Table list = new Table();
        if(seq == null || seq.isEmpty()){
            list.add(bnd("noactions"));
        }else{
            for(int pass = 0; pass < 2; pass++){
                boolean wantReverted = pass == 0;
                list.add(wantReverted ? bnd("reverted") : bnd("actions")).left().padTop(pass == 0 ? 2f : 6f).padBottom(4f).row();
                ...
                // main label
                row.add(Core.bundle.format("client.extraeditor.action", i + 1)).left().padRight(6f);

                // tiles affected
                row.add(Core.bundle.format("client.extraeditor.tiles", op.size())).left().padRight(6f);
                ...
                exp.clicked(() -> {
                    ...
                    if(expanded){
                        ...
                        try{
                            ...
                            preview.add("[red]" + bnd("previewfail"));
                        }
                ...
                }).size(34f).padLeft(6f).tooltip(bnd(isReverted ? "redo" : "revert"));
    ...
    private Table buildEraserOptionsTable(){
        Table pop = new Table(Tex.button);
        pop.defaults().left().pad(3f);
        pop.check(bnd("eraseblocks"), eraseBlocks, v -> eraseBlocks = v).row();
        pop.check(bnd("eraseores"), eraseOres, v -> eraseOres = v).row();
        pop.check(bnd("eraseteam"), eraseTeamOnly, v -> eraseTeamOnly = v).row();
        pop.pack();
        ui.editor.addChild(pop);
        Vec2 anchor = toolbar.localToStageCoordinates(new Vec2(toolbar.getWidth(), toolbar.getHeight()));
        Vec2 local = ui.editor.stageToLocalCoordinates(anchor);
    ...
    private void showMobilePasteBar(){
        ...
        }).tooltip(bnd("confirmpaste"));
        mobilePasteBar.button(Icon.cancel, Styles.flati, () -> setMode(Mode.none)).tooltip(Core.bundle.get("cancel"));
